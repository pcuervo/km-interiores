...
<article id="post-<?php the_ID(); ?>" <?php post_class($cmsms_project_size); echo (($project_categs != '') ? ' data-category="' . esc_attr($project_categs) . '"' : '') ?>>
	...
	<?php 
		cmsmasters_thumb_rollover(get_the_ID(), $project_img_size, false, true, true, false, $cmsms_project_video_type, $cmsms_project_video_link, $cmsms_project_video_links, false, true, $cmsms_project_link_redirect, $cmsms_project_link_url);
		...
		if ($title || $categories || $excerpt || $likes || $comments) {
			echo '<div class="project_inner">';
				...
				if ($title || $categories) {
					echo '<div class="cmsms_project_head_wrap">';
					
						($title) ? cmsmasters_project_heading(get_the_ID(), 'h5', true, $cmsms_project_link_redirect, $cmsms_project_link_url) : '';
